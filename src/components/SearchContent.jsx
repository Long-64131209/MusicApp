...
const SearchContent = ({ songs }) => {
  ...
  // Hack: Cập nhật song map toàn cục để Player phát được nhạc từ Search
  useEffect(() => {
      const songMap = {};
      songs.forEach(song => songMap[song.id] = song);
      // Merge với map cũ để không mất dữ liệu của trang khác
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 animate-in slide-in-from-bottom-4 duration-500">
      {songs.map((song, idx) => (
        <div 
          ...
          className="group relative bg-neutral-100/50 dark:bg-black/40 border border-neutral-200 dark:border-white/5 p-4 rounded-xl hover:bg-emerald-500/10 dark:hover:bg-white/10 transition cursor-pointer overflow-hidden flex flex-col gap-3 hover:border-emerald-500/50"
        >
           {/* IMAGE */}
           ...
           <div className="flex flex-col gap-1">
              <h3 className="font-bold text-neutral-800 dark:text-white font-mono truncate text-lg group-hover:text-emerald-500 transition">
                {song.title}
              </h3>
              <p className="text-xs text-neutral-500 dark:text-neutral-400 font-mono truncate uppercase tracking-wider flex items-center gap-2">
           ...
           <span className="absolute top-2 right-4 text-[40px] font-bold font-mono text-neutral-200 dark:text-white/5 pointer-events-none">
